...
def organ_foundry(size):
    ...
    for k in range(40):
        x, y = dx + s.rng.uniform(-150, 150), dy + s.rng.uniform(-150, 150)
        s.circ(x, y, s.rng.uniform(7, 11), 0.22, 0.45)
        s.dot(x + 1, y + 1, 1.6, 0.25)
    tree(s, dx - 150, dy - 30, 8, 70, 6, spread=24, shrink=0.78, w=2.2)
    tree(s, dx + 150, dy + 40, 186, 70, 6, spread=24, shrink=0.78, a=0.55, w=2.0, color=WHITE)
    s.end_clip()
    ...
    s.table(rx - 170, s.H - 190, [
        ("PRINT TIME", "38 h"), ("MATURATION", "21 d"), ("CELLS PLACED", "1.1 × 10¹¹"),
        ("VESSEL LENGTH", "160 km"), ("OUTPUT", "1 ORGAN / 23 d"),
    ], key_w=104)

    # layer plan, right
    qx, qy = rx, 330
    s.detail_ring(qx, qy, 170)
    for i, k in enumerate((1.0, 0.86, 0.72, 0.58, 0.44)):
        s.poly(bean(qx + 12, qy, 118 * k, stretch=1.0, notch=0.30 + 0.05 * i), 0.9 if i == 0 else 0.4, 1.0 if i == 0 else 0.5)
    for k in range(26):
        ang, rr = s.rng.uniform(-150, 150), s.rng.uniform(20, 96)
        x, y = polar(qx + 22, qy, rr, ang)
        s.circ(x, y, s.rng.choice([2, 2.5, 3.5, 5]), 0.9, 0.7, color=ARC if k % 3 else WHITE)
    s.ln(qx - 190, qy, qx + 190, qy, 0.15, 0.45, dash=[18, 4, 3, 4])
    s.view_label(qx, qy + 210, "C", "LAYER 1 212", "PLAN / VESSELS IN SECTION")
    return s
...
def quantum_simulator(size):
    ...
    # one logical qubit, left
    n, cell = 11, 26
    gx, gy = lx - cell * (n - 1) / 2, 300 - cell * (n - 1) / 2
    for i in range(n - 1):
        for j in range(n - 1):
            col = ARC if (i + j) % 2 else WHITE
            s.rect(gx + i * cell, gy + j * cell, cell, cell, 0.25, 0.4, fill=0.10 if (i + j) % 2 else 0.04, color=col)
            s.circ(gx + (i + 0.5) * cell, gy + (j + 0.5) * cell, 2.6, 0.7, 0.5)
    for i in range(n):
        for j in range(n):
            s.dot(gx + i * cell, gy + j * cell, 2.4, 0.95)
    s.ln(gx - 14, gy + 5 * cell, gx + (n - 1) * cell + 14, gy + 5 * cell, 0.9, 1.4, color=GOLD)
    s.view_label(lx, 300 + cell * (n - 1) / 2 + 44, "B", "ONE LOGICAL QUBIT", "241 PHYSICAL QUBITS / CODE DISTANCE 11")

    s.chart(rx - 170, 600, 260, 150, "LOGICAL ERROR RATE",
            lambda t: 0.03 + 0.93 * math.exp(-4.2 * t), "CODE DISTANCE 3 – 25", "LOG SCALE")
    s.table(rx - 170, s.H - 190, [
        ("LOGICAL QUBITS", "10 200"), ("PHYSICAL QUBITS", "2.5 × 10⁶"), ("BASE TEMPERATURE", "8 mK"),
        ("COOLING AT BASE", "30 µW"), ("WALL POWER", "180 kW"),
    ], key_w=124)
...
def tether_climber(size):
    ...
    # the whole system, left
    ex, top = lx, 150
    s.c.save()
    s.c.rectangle(lx - 260, 0, 520, 680)
    s.c.clip()
    s.circ(ex, 1270, 650, 0.9, 1.1, fill=0.04)

    def earth():
        s.c.new_sub_path()
        s.c.arc(ex, 1270, 650, 0, 2 * math.pi)
    s.hatch(earth, 6, 30, 0.14)
    s.c.restore()
    s.ln(ex, 620, ex, top, 0.85, 0.8)
    ...
    s.table(rx - 170, s.H - 190, [
        ("PAYLOAD", "20 t"), ("CLIMB SPEED", "200 km/h"), ("ANCHOR TO GEO", "7.5 d"),
        ("BEAM POWER", "4 MW"), ("COST TO ORBIT", "40 $ / kg"),
    ], key_w=110)

    # ribbon lattice, right
    qx, qy = rx, 330
    s.detail_ring(qx, qy, 170)
    s.begin_clip_circle(qx, qy, 169)
    a = 15.0
    for layer, (ox, oy, al) in enumerate(((0, 0, 0.75), (a * 0.8, a * 0.5, 0.18))):
        row = -14
        while row < 15:
            colm = -14
            while colm < 15:
                hx = qx + ox + colm * a * math.sqrt(3) + (a * math.sqrt(3) / 2 if row % 2 else 0)
                hy = qy + oy + row * a * 1.5
                s.poly([polar(hx, hy, a, 30 + 60 * k) for k in range(6)], al, 0.55 if layer == 0 else 0.4)
                colm += 1
            row += 1
    s.end_clip()
    s.view_label(qx, qy + 210, "C", "RIBBON LATTICE", "NANOTUBE WALL / NO DEFECT IN 100 000 km")
    return s
...
def cortical_mesh(size):
    ...
    # section through the head, left
    x0, x1 = lx - 210, lx + 210

    def scalp():
        s.c.rectangle(x0, 150, 420, 28)
    s.hatch(scalp, 5, 45, 0.3)
    s.rect(x0, 150, 420, 28, 0.7, 0.6)
    for xa, xb in ((x0, lx - 54), (lx + 54, x1)):
        s.rect(xa, 178, xb - xa, 62, 0.85, 0.8, fill=0.07)
        for k in range(int((xb - xa) // 12)):
            for q in range(4):
                s.dot(xa + 8 + k * 12 + (6 if q % 2 else 0), 188 + q * 14, 0.9, 0.35)
    s.rect(lx - 50, 172, 100, 64, 0.95, 1.1, fill=0.16)
    s.rect(lx - 36, 184, 72, 22, 0.7, 0.5, color=ARC)
    s.ln(x0, 246, x1, 246, 0.8, 0.7)
    s.ln(x0, 250, x1, 250, 0.4, 0.5)
    for k in range(6):
        pts = [(x, 270 + k * 24 + 9 * math.sin(x / 30.0 + k * 0.35)) for x in range(int(x0), int(x1) + 1, 6)]
        s.poly(pts, 0.85 - k * 0.12, 0.9 if k == 0 else 0.5, close=False)
    for k in range(22):
        x = x0 + 10 + k * 19
        s.ln(x, 410, x + s.rng.uniform(-6, 6), 450, 0.2, 0.4)
    for k in range(-6, 7):
        tx, ty = lx + k * 27, 330 + 50 * math.cos(k * 0.5) + s.rng.uniform(-10, 10)
        s.bez((lx + k * 3, 236), (lx + k * 5, 262), (tx, 262), (tx, ty), 0.85, 0.6, color=ARC)
        s.dot(tx, ty, 2, 0.95, ARC)
    for y, lab in ((164, "SCALP"), (209, "SKULL"), (248, "DURA"), (300, "CORTEX, LAYERS 1 – 6"), (430, "WHITE MATTER")):
        s.ln(x1 + 4, y, x1 + 16, y, 0.5, 0.5)
        s.text(lab, x1 + 22, y + 3, 6.5, a=0.6)
    s.view_label(lx, 500, "B", "SECTION", "SCALE 2 : 1")
    ...
    s.table(rx - 170, s.H - 190, [
        ("CHANNELS", "1 048 576"), ("SAMPLING", "20 kHz PER SITE"), ("LINK", "800 Mbit/s"),
        ("IMPLANT POWER", "15 mW"), ("SERVICE LIFE", "40 YEARS"),
    ], key_w=110)

    # one node among neurons, right
    qx, qy = rx, 330
    s.detail_ring(qx, qy, 170)
    s.begin_clip_circle(qx, qy, 169)
    for cx_, cy_, ang in ((qx - 80, qy - 70, 200), (qx + 96, qy - 40, 330), (qx + 40, qy + 100, 80), (qx - 100, qy + 80, 140)):
        s.poly([polar(cx_, cy_, 16, ang + d) for d in (0, 130, 230)], 0.55, 0.6, fill=0.08)
        for d in (0, 130, 230):
            x, y = polar(cx_, cy_, 16, ang + d)
            tree(s, x, y, ang + d, 34, 4, spread=26, shrink=0.78, a=0.4, w=0.6, color=WHITE)
    for off in (-5, 5):
        s.ln(qx - 180, qy + off - 30, qx + 180, qy + off + 30, 0.9, 0.8)
        s.ln(qx + off - 20, qy - 180, qx + off + 40, qy + 180, 0.9, 0.8)
...
def fusion_transport(size):
    ...
    # transit, left
    sx_, sy_ = lx - 40, 320
    s.dot(sx_, sy_, 5, 0.9, SUN)
    s.circ(sx_, sy_, 12, 0.4, 0.5, color=SUN)
    s.circ(sx_, sy_, 100, 0.55, 0.6)
    s.circ(sx_, sy_, 152, 0.55, 0.6)
    e = polar(sx_, sy_, 100, 150)
    m_ = polar(sx_, sy_, 152, 62)
    s.bez(e, (e[0] - 10, e[1] + 60), (m_[0] - 80, m_[1] + 30), m_, 0.95, 1.3, color=ARC)
    s.ellipse(sx_ - 26, sy_, 126, 120, rot=0, a0=0, a1=180, a=0.3, w=0.5, dash=[3, 4])
    s.circ(*e, 5, 0.95, 0.8, fill=0.5)
    ...
    s.table(rx - 170, s.H - 190, [
        ("CREW", "6"), ("DRY MASS", "140 t"), ("FUSION POWER", "75 MW"),
        ("THRUST", "900 N"), ("EXHAUST SPEED", "100 km/s"),
    ], key_w=110)

    # front view, right
    qx, qy = rx + 40, 300
    s.ln(qx, qy - 200, qx, qy + 200, 0.9, 0.9)
    for sgn in (-1, 1):
        s.rect(qx - 7, qy + sgn * 200 - 4, 14, 8, 0.95, 0.8, fill=0.3)
    s.circ(qx, qy, 96, 0.95, 1.2)
    s.circ(qx, qy, 86, 0.6, 0.6)
    for d in (45, 135, 225, 315):
        s.ln(*polar(qx, qy, 18, d), *polar(qx, qy, 86, d), 0.7, 0.6)
    for sgn in (-1, 1):
        s.circ(qx + sgn * 34, qy, 24, 0.85, 0.8, fill=0.06)
...
def air_refinery(size):
    ...
    # pellet from an enzyme bed, left
    dx, dy = lx, 300
    s.detail_ring(dx, dy, 150)
    s.circ(dx, dy, 104, 0.95, 1.1, fill=0.05)
    s.begin_clip_circle(dx, dy, 103)
    for k in range(70):
        x, y = dx + s.rng.uniform(-104, 104), dy + s.rng.uniform(-104, 104)
        s.circ(x, y, s.rng.uniform(4, 13), 0.4, 0.45)
    for k in range(16):
        x, y = dx + s.rng.uniform(-80, 80), dy + s.rng.uniform(-80, 80)
        pts = [(x, y)]
        ang = s.rng.uniform(0, 360)
        for q in range(14):
            ang += s.rng.uniform(-70, 70)
    ...
    s.table(rx - 170, s.H - 190, [
        ("CO₂ FIXED", "40 000 t / YEAR"), ("FUEL MADE", "13 000 t / YEAR"), ("WATER USED", "46 t / DAY"),
        ("SUNLIGHT TO FUEL", "21 %"), ("MIRROR FIELD", "40 ha"),
    ], key_w=120)

    # process, right
    bw, bh = 132, 32
    c1, c2, bus = rx - 190, rx - 10, rx + 176

    def box(x, y, label, color=WHITE, fill=0.05):
        s.rect(x, y, bw, bh, 0.9, 0.8, fill=fill, color=color)
        s.text_mid(label, x + bw / 2, y + bh / 2, 7, a=0.9, align="c")
    rows = ((120, "AIR", "CONTACTOR", "CO₂"), (196, "WATER", "ELECTROLYSER", "H₂"), (272, "SUNLIGHT", "RECEIVER", "HEAT"))
    for y, a_, b_, out in rows:
        box(c1, y, a_)
        box(c2, y, b_)
        s.arrow(c1 + bw, y + bh / 2, c2 - 2, y + bh / 2, 0.8, 0.7, head=6)
        s.ln(c2 + bw, y + bh / 2, bus, y + bh / 2, 0.8, 0.7, color=ARC)
        s.text(out, c2 + bw + 8, y + bh / 2 - 5, 6.5, a=0.85, color=ARC)
    s.ln(bus, 120 + bh / 2, bus, 364 + bh / 2, 0.8, 0.7, color=ARC)
    s.arrow(bus, 364 + bh / 2, c2 + bw + 2, 364 + bh / 2, 0.8, 0.7, head=6, color=ARC)
    box(c2, 364, "ENZYME BEDS", ARC)
    s.arrow(c2 + bw / 2, 364 + bh, c2 + bw / 2, 438, 0.8, 0.7, head=6)
    box(c2, 440, "CATALYST BEDS")
    s.arrow(c2 + bw / 2, 440 + bh, c2 + bw / 2, 514, 0.8, 0.7, head=6)
    box(c2, 516, "JET FUEL", GOLD, 0.12)
    s.poly([(c2 + 20, 196 + bh), (c2 + 20, 196 + bh + 14), (c1 + bw / 2, 196 + bh + 14)], 0.5, 0.6, close=False)
    s.text("O₂ VENTED", c1 + bw / 2 - 6, 196 + bh + 17, 6, a=0.5, align="r")
    s.view_label(rx, 610, "C", "PROCESS", "WHAT GOES IN, WHAT COMES OUT")
    return s
